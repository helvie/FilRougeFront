...
export class EditFormationComponent implements OnInit {
  ...
  isSelected(subTheme: SubTheme): boolean {
    return this.listSubthemeSelected.some(
      (selectedSubTheme) => selectedSubTheme.id === subTheme.id
    );
  }
  ...
  selectImage(event: any): void {
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        this.imageSrc =
          test.changingThisBreaksApplicationSecurity?.split(',')[1];
      };
  ...
  updateTraining() {
    ...
    this.trainingService
      .edit(trainingId, trainingUpdate)
      .pipe(
        tap(
          (value) => {
          ...
          (error) => {
            ...
            if (error.error == null) {
              this.toastService.alertError(
                "Une erreur est survenue lors de l'enregistrement d'une formation"
              );
              this.isFormTrainingLoading = false;
            } else {
